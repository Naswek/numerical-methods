...
class GaussSecond extends Interpolator {
  override def name: String = "Многочлен Гаусса (вторая формула)"
  ...
  override def solve(points: Seq[Point], targetX: Double): InterpolationResult = {
    ...
    for (k <- 1 until math.min(midIdx + 1, n - midIdx)) {
      tPart *= (t - k + 1)
      factorial *= (2 * k - 1)
      res += (tPart * table(midIdx - k)(2 * k - 1)) / factorial

      tPart *= (t + k)
      factorial *= (2 * k)
      res += (tPart * table(midIdx - k)(2 * k)) / factorial
    }
    InterpolationResult(name, res, table, Message.Success, equation = InterpolationResult.getGaussSecondEquation(points, table))
  }
}
