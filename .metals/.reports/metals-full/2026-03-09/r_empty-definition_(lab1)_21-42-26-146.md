...
non-local guesses:
	 -Colors.Colors.printSuccess.
	 -Colors.Colors.printSuccess#
	 -Colors.Colors.printSuccess().
	 -IOHelper.Colors.printSuccess.
	 -IOHelper.Colors.printSuccess#
	 -IOHelper.Colors.printSuccess().
	 -Colors.printSuccess.
	 -Colors.printSuccess#
	 -Colors.printSuccess().
	 -scala/Predef.Colors.printSuccess.
	 -scala/Predef.Colors.printSuccess#
	 -scala/Predef.Colors.printSuccess().
offset: 3790
...
class MatrixSolver {
    ...
    def runJacobiIterationsWithFormula(C: Array[Array[Double]], d: Array[Double], printErrors: Boolean = false): Unit = {
        ...
        // Вспомогательная функция для индексов в нижнем индексе
        def idx(num: Int): String = {
            val sub = "₀₁₂₃₄₅₆₇₈₉"
            num.toString.map(c => sub(c.asDigit)).mkString
        }

        // Печать формулы Якоби для каждой строки
        for (i <- 0 until n) {
            ...
            Colors.printSuccess(formula)
        }

        // Получаем эпсилон и начальное приближение
        ...
        while (diff > epsilon) {
            ...
            Colors.@@printSuccess(s"Погрешности на итерации $count: ${diffVector.mkString(", ")}")
            }
